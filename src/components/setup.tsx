...
import { generateSalt, hashPassword, generateId, saveToStorage } from '@/lib/utils';
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <Card className="w-full max-w-2xl">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Add trusted locations where you can access your passwords. You must be within these zones to unlock your vault.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {currentLocation && (
            <div className="space-y-4 p-4 border rounded-lg">
              ...
              <Button onClick={addTrustedLocation} disabled={!locationName.trim()}>
          ...
          {trustedLocations.length > 0 && (
            <div className="space-y-4">
              <Separator />
              <h3 className="font-medium">Trusted Locations ({trustedLocations.length})</h3>
              <div className="space-y-2">
                {trustedLocations.map((location) => (
                  <div key={location.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {location.radius}m radius • {location.latitude.toFixed(4)}, {location.longitude.toFixed(4)}
                      </p>
                    </div>
                    <Button
                      variant="outline"
          ...
          <div className="flex justify-between pt-4">
            ...
            <Button onClick={completeSetup} disabled={trustedLocations.length === 0}>
              Complete Setup
            </Button>
          </div>
        </CardContent>
